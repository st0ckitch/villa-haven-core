...
const EquestrianManagement = () => {
  ...
  useEffect(() => {
    (async () => {
      const [settingsRes, rendersRes] = await Promise.all([
        ...
        supabase.from("project_renders").select("*").eq("project", "equestrian").order("sort_order"),
      ]);
      if (settingsRes.data) {
  ...
  const handleUpload = async (key: string, file: File) => {
    ...
    const path = `equestrian/${key}-${Date.now()}.${ext}`;
  ...
  const handleRenderUpload = async (file: File) => {
    ...
    const ext = file.name.split(".").pop();
    const path = `equestrian/render-${Date.now()}.${ext}`;
    const { error } = await supabase.storage.from("renders").upload(path, file);
    ...
    const { data } = await supabase.from("project_renders").insert({
      project: "equestrian", title: "", image_url: urlData.publicUrl, sort_order: renders.length,
    }).select().single();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Equestrian page saved" });
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-serif mb-6">Equestrian Club Page</h1>
      ...
      <Button onClick={handleSave} disabled={saving} className="mt-6">
        ...
        Save Equestrian Page
      </Button>
    </div>
  );
};

export default EquestrianManagement;
